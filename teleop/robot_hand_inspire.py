        ...
        try:
            while self.running:
                ...
                if not np.all(right_hand_mat == 0.0) and not np.all(left_hand_mat[4] == np.array([-1.13, 0.3, 0.15])): # if hand data has been initialized.
                    ...
                    left_q_target  = self.left_retargeting.retarget(ref_left_value)[self.left_dex_retargeting_to_hardware]
                    right_q_target = self.right_retargeting.retarget(ref_right_value)[self.right_dex_retargeting_to_hardware]
